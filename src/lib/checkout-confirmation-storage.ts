...
export function persistLastCheckoutConfirmation(
	payload: LastCheckoutConfirmationPayload,
): void {
	try {
...
export function consumeLastCheckoutConfirmation(
	orderId: string,
): LastCheckoutConfirmationPayload | null {
	try {
		const raw = sessionStorage.getItem(STORAGE_KEY);
		if (!raw) return null;
		const parsed = JSON.parse(raw) as LastCheckoutConfirmationPayload;
		if (!parsed || typeof parsed !== "object" || parsed.orderId !== orderId)
			return null;
