...
const AVATAR_PALETTES = [
	...
	{
		bg: "color-mix(in srgb, var(--color-cream-dark) 80%, var(--color-paper))",
		text: "var(--color-ink)",
	},
	{
		bg: "color-mix(in srgb, var(--color-cream-dark) 65%, var(--color-paper))",
		text: "var(--color-text-dark)",
	},
	{
		bg: "color-mix(in srgb, var(--color-cream-dark) 55%, var(--color-paper))",
		text: "var(--color-ink)",
	},
	{
		bg: "color-mix(in srgb, var(--color-cream-dark) 72%, var(--color-paper))",
		text: "var(--color-ink)",
	},
	{
		bg: "color-mix(in srgb, var(--color-cream-dark) 60%, var(--color-paper))",
		text: "var(--color-text-dark)",
	},
];
		...
		<article className="group relative flex flex-col overflow-hidden border border-cream-dark bg-white transition-shadow hover:shadow-md">
			...
			<div className="flex flex-1 flex-col gap-4 p-6">
				...
				<div className="flex items-start justify-between gap-3">
					<div className="flex min-w-0 flex-1 items-start gap-3">
						{profile.profileImage ? (
							...
							<div
								className="flex h-12 w-12 shrink-0 items-center justify-center rounded-sm font-bold font-display text-lg"
						...
						<div className="min-w-0 flex-1">
							<h2
								className="font-bold font-display text-text-dark uppercase leading-snug transition-colors group-hover:text-primary"
					...
					{/* Verified badge */}
					<span
						className="shrink-0 border px-2.5 py-1 font-body font-bold text-[10px] uppercase tracking-widest"
						style={{
				...
				{profile.publicTagline ? (
					<p className="line-clamp-2 font-body text-[13px] text-text-dark/80 leading-relaxed">
						{profile.publicTagline}
					</p>
				) : null}
				...
				<div className="flex flex-wrap items-center gap-2">
					<span
						className="inline-flex items-center gap-1.5 px-3 py-1 font-body font-semibold text-[11px]"
						style={{
							background:
					...
					>
						...
						{productCount === 1
							? interpolate(t("partnersDirectory.listingOne"), {
									count: productCount,
								})
							: interpolate(t("partnersDirectory.listingMany"), {
									count: productCount,
								})}
					</span>
					{profile.entityType ? (
				...
				{categories.length > 0 ? (
					<div className="flex flex-wrap gap-1.5">
						{categories.slice(0, 4).map((cat) => (
							<span
								className="border border-cream-dark bg-white px-2.5 py-1 font-body font-medium text-[11px] text-text-dark/80"
								key={cat}
							>
								{cat}
							</span>
				...
				{websiteLabel ? (
					<p
						className="truncate font-body text-[11px] text-text-muted"
						title={profile.website ?? undefined}
					>
						{websiteLabel}
					</p>
				) : null}
				...
				{/* CTA */}
				<div className="mt-auto border-cream-dark border-t pt-4">
					<Link
						className="inline-flex w-full items-center justify-center gap-2 bg-primary px-4 py-2.5 font-bold font-display text-[13px] text-white uppercase tracking-[0.1em] transition-opacity hover:opacity-90"
...
export default function PartnersClient({ partners }: { partners: Partner[] }) {
	...
	const regions = useMemo(() => {
		const set = new Set(
			partners.map((p) => p.profile?.region).filter(Boolean) as string[],
		);
	...
	return (
		<section className="flex-1 bg-cream">
			{/* Filter bar */}
			<div className="border-cream-dark border-b bg-white px-4 py-5">
				<AnimateOnScroll
					className="mx-auto flex max-w-7xl flex-col items-start gap-4 sm:flex-row sm:items-center"
					direction="up"
				>
					<div className="relative w-full shrink-0 sm:w-72">
						<svg
							aria-hidden
							className="absolute top-1/2 left-3.5 -translate-y-1/2 text-text-muted/50"
							fill="none"
							height="14"
							viewBox="0 0 16 16"
							width="14"
						...
						<input
							className="w-full border border-cream-dark bg-white py-2.5 pr-4 pl-9 font-body text-sm text-text-dark transition-colors placeholder:text-text-muted/50 focus:border-primary focus:outline-none"
					...
					<div
						aria-hidden
						className="hidden h-6 w-px shrink-0 bg-cream-dark sm:block"
					/>

					<div className="flex flex-wrap items-center gap-2">
						<span className="me-1 font-body font-bold text-[10px] text-text-muted uppercase tracking-[0.15em]">
				...
				{filtered.length === 0 ? (
					<div className="py-20 text-center">
						<p className="font-body text-text-muted">
							{t("partnersDirectory.emptyFilters")}
						</p>
					</div>
				) : (
