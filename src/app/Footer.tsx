...
): { heading: string; items: FooterItem[] }[] {
...
export default async function Footer() {
	...
	return (
		<footer className="border-cream-dark border-t bg-paper">
				...
				>
					<div className="flex flex-col gap-5">
						<Link className="flex flex-col gap-1" href="/">
							...
							>
								{SHOW_MULTI_PRODUCER_EXPERIENCE
									? t("footer.taglineMulti")
									: NEVALI_HOUSE_BRAND.lineUnderLogo}
							</span>
						</Link>
						<p
							className="max-w-[220px] font-sans text-text-muted text-xs leading-relaxed"
						...
						<div className="mt-1 flex items-center gap-3">
							{SOCIAL.map((s) => (
								<a
									aria-label={s.label}
									className="border border-primary/35 bg-white px-2 py-1 font-bold font-sans text-primary-dark text-xs uppercase tracking-wider transition-colors duration-200 hover:border-primary hover:text-primary-darker"
									href={s.href}
									key={s.label}
			...
			<AnimateOnScroll
				className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-2 border-cream-dark border-t px-6 py-4 sm:flex-row sm:items-center"
				delay={80}
				direction="up"
			>
				...
				<div className="flex flex-wrap items-center gap-3 sm:gap-4">
					<LocaleSwitcher />
					<Link
						className="font-sans text-text-muted text-xs transition-colors duration-200 hover:text-primary-darker"
						href="/privacy"
					>
