...
export default function FAQSection() {
  ...
  const faqs: FaqItem[] = useMemo(() => {
    const raw = SHOW_MULTI_PRODUCER_EXPERIENCE ? messages.faq.multi : messages.faq.single;
    return raw.map((item) => ({
      ...
      <div className="mx-auto flex w-full max-w-[1200px] flex-col gap-12 px-4 sm:px-6 lg:gap-16 lg:px-12">
        <AnimateOnScroll className="flex flex-col items-center gap-5 text-center" direction="up" scale>
          ...
          <p className="max-w-[480px] font-sans text-lg text-text-muted leading-relaxed">
            {SHOW_MULTI_PRODUCER_EXPERIENCE ? t("faq.introMulti") : t("faq.introSingle", { brand })}
          </p>
        </AnimateOnScroll>

        <AnimateOnScroll className="mx-auto flex w-full max-w-[820px] flex-col" delay={100} direction="up">
          {faqs.map((faq, i) => (
            <div className="border-b border-cream-dark" key={faq.q}>
              <button
                className="flex w-full items-center justify-between gap-6 py-7 text-start"
                onClick={() => setOpen(open === i ? -1 : i)}
                type="button"
              >
                <span className="font-sans font-semibold text-lg text-text-dark leading-snug">{faq.q}</span>
                <div
                  className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border"
                  style={{ borderColor: "color-mix(in srgb, var(--color-gold) 45%, var(--color-cream-dark))" }}
                >
                  <span className="text-base text-secondary leading-none" style={{ marginTop: -1 }}>
                    {open === i ? "−" : "+"}
                  </span>
                </div>
              </button>
              {open === i ? (
                <p className="max-w-[720px] pb-7 font-sans text-base text-text-muted leading-relaxed">{faq.a}</p>
              ) : null}
            </div>
          ))}
          ...
          >
            <div>
              <p className="mb-0.5 font-sans font-semibold text-base text-text-dark">{t("faq.stillTitle")}</p>
              <p className="font-sans text-sm text-text-muted">{t("faq.stillBody")}</p>
            </div>
            <button
            ...
            >
              ...
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <path d="M2 7h10M8 3l4 4-4 4" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" />
              </svg>
            </button>
          </div>
        </AnimateOnScroll>
