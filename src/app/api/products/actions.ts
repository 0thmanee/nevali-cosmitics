...
export async function setProductImageVariant(
	input: SetProductImageVariantInput,
) {
	const orgId = await getProducerOrgId();
	if (!orgId) throw new Error("You must belong to an organization.");
...
export async function createProduct(data: CreateProductInput) {
	const orgId = await getProducerOrgId();
	if (!orgId)
		throw new Error("You must belong to an organization to create products.");
...
export async function updateProduct(
	...
	const orgId = await getProducerOrgId();
	if (!orgId)
		throw new Error("You must belong to an organization to update products.");
	...
	return updateProductRepo(productId, orgId, {
		...
		capacity: parsed.capacity ?? null,
		description: parsed.description ?? null,
		...(parsed.featuredOnHome !== undefined
...
export async function setProductStatus(input: SetProductStatusInput) {
	...
	if (productMeta) {
		const recipients = await getOrganizationMemberEmails(
			productMeta.organizationId,
		);
