...
import type { CertificationRow, CertificationWithProductRow } from "../schemas/certifications.schema";
...
export async function listCertificationsByOrgRepo(
  ...
  const where: { organizationId: string; productId?: string | null } = { organizationId };
...
export async function deleteCertificationRepo(
  ...
  if (!cert) throw new Error("Certification not found or does not belong to your organization.");
...
export async function listCertificationsForAdminRepo(filters: {
  ...
  const where: { status?: string; productId?: string | null; product?: { organizationId: string; id?: string } } = {};
