...
import type { PublicProducerProfile } from "./schemas/profile.schema";
import {
  getProfileByUserId,
  upsertProfileByUserId,
  setProfileCompleted,
  updateProfileImageRepo,
  getPublicProducerByOrgSlug,
  getOrganizationSlugByUserId,
...
export async function getRedirectPathAfterAuth(): Promise<string> {
  ...
  if (role === "buyer" && status === "enabled") return "/buyer";
  if (role === "partner" && status === "enabled") return "/artisan";
...
/** Public artisan profile by org slug (no auth). */
export async function getPublicProducerBySlug(slug: string): Promise<PublicProducerProfile | null> {
  return getPublicProducerByOrgSlug(slug);
}

/** Organization slug for shareable public profile URL (`/artisans/[slug]`). */
export async function getMyOrganizationSlug(): Promise<string | null> {
  const session = await getSession();
  if (!session?.user?.id) return null;
  return getOrganizationSlugByUserId(session.user.id);
}
