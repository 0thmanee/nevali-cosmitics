...
export async function listPartnersRepo(filters: { organizationId?: string } = {}) {
  ...
  if (filters.organizationId) where.members = { some: { organizationId: filters.organizationId } };
  return prisma.user.findMany({
    where,
...
export async function listPublicPartnersRepo(): Promise<PublicPartnerListItem[]> {
