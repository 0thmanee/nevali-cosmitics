...
export async function POST(request: Request) {
	try {
		...
		if ("error" in result) {
			...
			if (status === 503 && /not configured|supabase/i.test(message)) {
				message +=
					" Set SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, and SUPABASE_STORAGE_BUCKET in .env for local media (see README).";
			}
