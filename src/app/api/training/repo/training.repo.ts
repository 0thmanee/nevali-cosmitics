...
export async function getProgramWithModulesAndMediaForAdminRepo(
	programId: string,
): Promise<{
...
export async function deleteTrainingProgramRepo(
	programId: string,
): Promise<void> {
...
export async function deleteTrainingModuleRepo(
	moduleId: string,
): Promise<void> {
...
export async function deleteTrainingProgramMediaRepo(
	mediaId: string,
): Promise<void> {
...
export async function updateEnrollmentProgressRepo(
	...
	const progress =
		totalModules > 0
			? Math.min(100, Math.round((data.modulesCompleted / totalModules) * 100))
			: 0;
	const status =
		data.status ??
	...
	if (data.modulesCompleted > 0 && !enrollment.startedAt)
		updateData.startedAt = new Date();
	if (status === TRAINING_ENROLLMENT_STATUSES[2])
