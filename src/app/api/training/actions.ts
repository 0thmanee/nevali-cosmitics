...
export async function enrollInProgram(programId: string) {
  const organizationId = await getProducerOrgId();
  if (!organizationId) throw new Error("You must belong to an organization.");
  const existing = await getEnrollmentByOrgAndProgramRepo(organizationId, programId);
  if (existing) return existing;
  const program = await getTrainingProgramByIdRepo(programId);
  if (!program || program.status !== "PUBLISHED") throw new Error("Program not found or not available.");
  return createEnrollmentRepo({ organizationId, programId });
}

/** Update progress for an enrollment (current org only). */
export async function updateMyEnrollmentProgress(input: UpdateEnrollmentProgressInput) {
  const organizationId = await getProducerOrgId();
  if (!organizationId) throw new Error("You must belong to an organization.");
...
export async function listTrainingProgramsForAdmin(filters: { status?: "DRAFT" | "PUBLISHED" } = {}) {
  try {
    await requireSuperadmin();
    const rows = await listTrainingProgramsForAdminRepo(filters);
    return JSON.parse(JSON.stringify(rows)) as Awaited<ReturnType<typeof listTrainingProgramsForAdminRepo>>;
  } catch (err) {
...
export async function getAdminTrainingCounts(): Promise<{ ALL: number; PUBLISHED: number; DRAFT: number }> {
  await requireSuperadmin();
...
export async function updateTrainingProgram(programId: string, data: UpdateTrainingProgramInput) {
  await requireSuperadmin();
...
export async function listMediaForAdmin(programId: string, options: { moduleId?: string | null } = {}) {
  await requireSuperadmin();
  const rows = await listMediaByProgramIdRepo(programId, options);
  return JSON.parse(JSON.stringify(rows)) as typeof rows;
}

export async function addTrainingProgramMedia(data: CreateTrainingProgramMediaInput) {
  await requireSuperadmin();
...
export async function assignProgramToOrganization(input: CreateEnrollmentInput) {
  await requireSuperadmin();
  const parsed = createEnrollmentSchema.parse(input);
  const existing = await getEnrollmentByOrgAndProgramRepo(parsed.organizationId, parsed.programId);
  if (existing) return existing;
  const program = await getTrainingProgramByIdRepo(parsed.programId);
  if (!program) throw new Error("Program not found.");
