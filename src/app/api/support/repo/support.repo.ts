...
export async function listSupportTicketsForAdminRepo(filters: { organizationId?: string; status?: "OPEN" | "IN_REVIEW" | "RESOLVED" } = {}): Promise<SupportTicketAdminRow[]> {
...
export async function getSupportTicketForAdminRepo(
  ...
  if (statusEvents.length === 0) {
    ...
    const again = await prisma.supportTicket.findUnique({
      where: { id: ticketId },
      select: { statusEvents: { orderBy: { createdAt: "desc" }, select: statusEventSelect } },
    });
...
export async function getAdminSupportTicketCountsRepo(organizationId?: string | null): Promise<{
  ALL: number;
  OPEN: number;
  IN_REVIEW: number;
  RESOLVED: number;
...
export async function backfillStatusEventForTicketRepo(ticketId: string): Promise<void> {
  const ticket = await prisma.supportTicket.findUnique({
    where: { id: ticketId },
    select: { status: true, statusEvents: { take: 1, orderBy: { createdAt: "desc" }, select: { id: true } } },
  });
