...
async function requireProducerOrganizationId(): Promise<string> {
  ...
  if (!orgId) redirect("/auth/login?callbackUrl=" + encodeURIComponent("/artisan/support"));
  return orgId;
}
...
export async function listMySupportTickets() {
  ...
  return rows.map(({ adminNotes: _, ...r }) => ({ ...r, adminNotes: null })) as Awaited<ReturnType<typeof listTicketsByOrganizationId>>;
}
...
export async function listSupportTicketsForAdmin(filters: { organizationId?: string; status?: "OPEN" | "IN_REVIEW" | "RESOLVED" } = {}) {
  await requireSuperadmin();
  return listSupportTicketsForAdminRepo(filters);
}

/** Admin support ticket counts by status (optional organizationId). */
export async function getAdminSupportTicketCounts(organizationId?: string | null): Promise<{
  ALL: number;
  OPEN: number;
  IN_REVIEW: number;
  RESOLVED: number;
...
export async function updateSupportTicketAdminNotes(input: UpdateSupportTicketAdminNotesInput) {
  await requireSuperadmin();
