...
const FEATURED_I18N_KEY: Record<
	CategoryId,
	"arganOils" | "skincare" | "hammamBody" | "hairFragrance"
> = {
...
export function FeaturedCategorySectionClient({ samples }: Props) {
	...
	return (
		...
		>
			<div className="mx-auto flex max-w-7xl flex-col gap-10 px-6 lg:flex-row lg:gap-12">
				<AnimateOnScroll
					className="flex flex-[1.2] flex-col gap-5"
					direction="right"
				>
					<h2
						className="font-bold font-display text-text-dark uppercase leading-[1.0]"
						style={{
					...
					<p className="max-w-sm font-sans text-base text-text-muted leading-relaxed">
						{active.desc}
					</p>
					<div className="mt-2 flex flex-col gap-2">
						{active.details.map((d) => (
							<div className="flex gap-2 font-sans text-sm" key={d.label}>
				...
				<AnimateOnScroll
					className="flex flex-[1.5] flex-col gap-6 sm:flex-row"
					delay={90}
					direction="left"
					scale
				>
					<div className="flex shrink-0 flex-col gap-0 border-cream-dark border-l-2 sm:w-44">
						{CATEGORY_ORDER.map((id) => (
							<button
								className="px-4 py-2.5 text-left font-sans text-sm uppercase tracking-wider transition-colors"
								key={id}
								...
								style={{
									...
									borderLeft:
										activeId === id
											? "2px solid var(--color-primary-darker)"
					...
					>
						...
						{(() => {
							const imgSrc =
								sample?.firstImageUrl ??
								featuredCategoryPlaceholderImage(activeId, 900);
							return (
								<div
									...
									>
										<Image
											...
											style={{
												...
												filter:
													"drop-shadow(0 24px 40px color-mix(in srgb, var(--color-ink) 55%, transparent))",
											}}
										/>
									</div>
									<div
						...
						>
							<div className="pointer-events-auto flex flex-col gap-1.5">
								{sample ? (
									<>
										<Link
											className="line-clamp-2 font-sans font-semibold text-sm text-white hover:underline"
											href={`/products/${sample.id}`}
										>
											{sample.name}
										</Link>
										<p className="font-sans font-semibold text-sm text-white/95">
											{formatMad(sample.fromPriceMad)}
										</p>
									</>
								) : null}
