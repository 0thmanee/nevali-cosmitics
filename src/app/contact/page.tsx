...
export async function generateMetadata(): Promise<Metadata> {
	...
	return {
		...
		description: SHOW_MULTI_PRODUCER_EXPERIENCE
			? t("contact.metaDescMulti")
			: t("contact.metaDescSingle", { brand }),
	};
}
				...
				>
					<div className="flex flex-col gap-3">
						...
						<p className="font-sans text-[15px] text-text-muted leading-relaxed">
							{SHOW_MULTI_PRODUCER_EXPERIENCE
								? t("contact.introMulti")
								: t("contact.introSingle", { brand })}
						</p>
					</div>
					...
					<div className="grid w-full gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
						...
						<div className="rounded-sm border border-cream-dark bg-white p-6">
							<p className="font-sans font-semibold text-text-muted text-xs uppercase tracking-widest">
								{t("contact.directEmailLabel")}
							</p>
							<a
								className="mt-2 block break-all font-sans text-forest-light text-sm hover:underline"
							...
							<p className="mt-3 font-sans text-text-muted text-xs">
								{env.CONTACT_PUBLIC_EMAIL
									? t("contact.envHintConfigured")
									: t("contact.envHintDefault")}
							</p>
							<div className="mt-5 border-cream-dark border-t pt-4">
								<Link
									className="block font-sans text-text-muted text-xs hover:text-text-dark hover:underline"
									href="/privacy"
								>
									{t("footer.privacyPolicy")}
								</Link>
								<Link
									className="mt-2 block font-sans text-text-muted text-xs hover:text-text-dark hover:underline"
									href="/terms"
								>
