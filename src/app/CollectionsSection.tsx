...
function MosaicTile({ product }: { product: MosaicProduct }) {
  const src = product.firstImageUrl ?? productPlaceholderImageUrl(`${product.id}:${product.category}`, 900);
  return (
    ...
    <section className="w-full bg-white py-28">
      <div className="mx-auto max-w-7xl px-6">
        <AnimateOnScroll className="mb-8 flex flex-col gap-2" direction="up">
          <h2
            className="font-bold font-display uppercase leading-[1.0] text-text-dark"
          ...
          <p className="max-w-md font-sans text-base leading-relaxed text-text-muted">
            {SHOW_MULTI_PRODUCER_EXPERIENCE ? t("collections.subtitleMulti") : t("collections.subtitleSingle", { brand })}
          </p>
        </AnimateOnScroll>
        ...
        {products.length === 0 ? (
          <AnimateOnScroll className="flex flex-col items-start gap-3 rounded-sm border border-stone-200 bg-stone-50/80 px-6 py-10" direction="up">
            <p className="font-sans font-semibold text-text-dark">{t("collections.emptyTitle")}</p>
            <p className="max-w-md font-sans text-sm text-text-muted">
              {SHOW_MULTI_PRODUCER_EXPERIENCE ? t("collections.emptyBodyMulti") : t("collections.emptyBodySingle")}
            </p>
            <Link
              className="px-5 py-2.5 font-sans text-xs font-semibold uppercase tracking-wide text-white transition-opacity hover:opacity-90"
              href="/products"
