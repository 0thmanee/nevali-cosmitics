	...
	return (
		<div className="relative h-full w-full">
			...
			<div
				...
				style={{
					background:
						"linear-gradient(to top, color-mix(in srgb, var(--color-ink) 45%, transparent) 0%, transparent 68%)",
				}}
			>
				<span className="line-clamp-2 font-medium font-sans text-sm text-white/95 uppercase tracking-widest">
					{product.name}
				</span>
			</div>
		</div>
			...
			<div className="mx-auto max-w-7xl px-6">
				<AnimateOnScroll className="mb-8 flex flex-col gap-2" direction="up">
					...
					>
						...
						<br />
						{t("collections.titleLine2")}
					</h2>
					<p className="max-w-md font-sans text-base text-text-muted leading-relaxed">
						{SHOW_MULTI_PRODUCER_EXPERIENCE
							? t("collections.subtitleMulti")
							: t("collections.subtitleSingle", { brand })}
					</p>
				</AnimateOnScroll>

				{products.length === 0 ? (
					<AnimateOnScroll
						className="flex flex-col items-start gap-3 rounded-sm border border-stone-200 bg-stone-50/80 px-6 py-10"
						direction="up"
					>
						...
						<p className="max-w-md font-sans text-sm text-text-muted">
							{SHOW_MULTI_PRODUCER_EXPERIENCE
								? t("collections.emptyBodyMulti")
								: t("collections.emptyBodySingle")}
						</p>
						<Link
							className="px-5 py-2.5 font-sans font-semibold text-white text-xs uppercase tracking-wide transition-opacity hover:opacity-90"
							href="/products"
							style={{ background: "var(--color-ink)" }}
						>
							{t("collections.browseCatalog")}
						</Link>
					</AnimateOnScroll>
				) : (
					<AnimateOnScroll
