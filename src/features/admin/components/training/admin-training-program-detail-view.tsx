  ...
  return (
    <div className="p-4 lg:p-6 flex flex-col gap-6">
      ...
      <div className="rounded-2xl overflow-hidden shadow-sm" style={cardStyle}>
        ...
        <div className="p-6">
          {modules.length === 0 ? (
            <p className="font-sans text-sm text-[#7a4d38]">No modules yet. Add a module to define sections and attach documents.</p>
          ) : (
              ...
              {(modules as TrainingModuleRow[]).map((mod, i) => (
                ...
                >
                  <Link
                    ...
                    className="flex-1 min-w-0 font-sans text-sm font-semibold text-[#2a0f05] hover:text-[#7b2d1e] hover:underline truncate"
                  >
                    {i + 1}. {mod.title}
                  </Link>
                  <button
      ...
      <div className="rounded-2xl overflow-hidden shadow-sm" style={cardStyle}>
        <div className="px-6 py-4 border-b border-[#f0e8dc]">
          <h2 className="font-serif font-bold text-[15px] text-[#2a0f05]">Program-level documents</h2>
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap items-end gap-3">
            <div>
              ...
              <input
                ...
                className="font-sans text-sm rounded-xl px-3 py-2 border border-[#f0e8dc] w-40 bg-[#F5F0E8]"
              />
            </div>
            <button
              type="button"
          ...
          {(() => {
            ...
            return programMedia.length === 0 ? (
              <p className="font-sans text-sm text-[#7a4d38]">No program-level documents yet. Upload a video, PDF, or image above.</p>
            ) : (
            <ul className="flex flex-col gap-2">
              {programMedia.map((m) => (
      ...
      <div className="rounded-2xl overflow-hidden shadow-sm" style={cardStyle}>
        <div className="px-6 py-4 border-b border-[#f0e8dc]">
          <h2 className="font-serif font-bold text-[15px] text-[#2a0f05]">Assign to partner</h2>
          <p className="font-sans text-[11px] text-[#7a4d38] mt-0.5">Assign this program to an organization. They will see it in their Training page.</p>
        </div>
        <div className="p-6 flex flex-wrap items-center gap-3">
          <select
            value={assignOrgId}
            onChange={(e) => setAssignOrgId(e.target.value)}
            className="font-sans text-sm rounded-xl px-3 py-2 border border-[#f0e8dc] bg-[#F5F0E8] min-w-[200px]"
          >
      ...
      <Link href="/admin/training" className="font-sans text-sm font-medium text-[#7a4d38] hover:text-[#2a0f05] transition-colors w-fit">
        ← Back to Training
      </Link>
    </div>
  );
