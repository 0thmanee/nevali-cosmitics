...
export function ShopOrdersList() {
	...
	const subtitle = selectedSlug
		? interpolate(t("adminShopOrders.subtitleFiltered"), {
				count: orders.length,
			})
		: orders.length === 1
			? interpolate(t("adminShopOrders.subtitleAll"), { count: orders.length })
			: interpolate(t("adminShopOrders.subtitleAllPlural"), {
					count: orders.length,
				});
