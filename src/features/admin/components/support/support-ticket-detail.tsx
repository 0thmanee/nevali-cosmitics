...
const statusStyles: Record<string, { bg: string; color: string; border: string }> = {
  ...
  RESOLVED: { bg: "rgba(26,5,0,0.08)", color: "#7a4d38", border: "1px solid #f0e8dc" },
};
...
export function SupportTicketDetail({ ticketId }: Props) {
      ...
      <div className="p-4 lg:p-6">
        <div className="rounded-xl overflow-hidden" style={{ background: "white", border: "1px solid #f0e8dc" }}>
          <div className="px-5 py-12 flex items-center justify-center">
            <p className="font-sans text-sm text-[#7a4d38]">Loading ticket…</p>
          </div>
        </div>
      </div>
    );
  ...
  if (isError || !ticket) {
    return (
      <div className="p-4 lg:p-6">
        <div className="rounded-xl overflow-hidden px-5 py-6" style={{ background: "white", border: "1px solid #f0e8dc" }}>
          <p className="font-sans text-sm text-red-600">
            {error instanceof Error ? error.message : "Ticket not found."}
          </p>
          <Link href="/admin/support" className="font-sans text-sm text-[#2a0f05] hover:underline mt-2 inline-block">
            ← Back to Support
          </Link>
        </div>
      </div>
    ...
    <div className="p-4 lg:p-6 flex flex-col gap-4">
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: "white", border: "1px solid #f0e8dc" }}>
        <div className="px-5 py-4 border-b flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3" style={{ borderColor: "#f0e8dc" }}>
          <div>
            <h2 className="font-serif font-bold text-[15px] text-[#2a0f05]">{t.subject}</h2>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select
              ...
              style={{ borderColor: "#f0e8dc", background: "#FAFAF7", color: "#2a0f05" }}
            >
        ...
        <div className="p-5 flex flex-col gap-6">
          <section>
            <h3 className="font-sans text-[10px] font-bold tracking-wider text-[#7a4d38] uppercase mb-2">Message</h3>
            <div className="rounded-xl px-4 py-3" style={{ background: "#FAFAF7", border: "1px solid #f0e8dc" }}>
              <p className="font-sans text-[13px] text-[#2a0f05] leading-relaxed whitespace-pre-wrap">{t.message}</p>
            </div>
          </section>

          <section>
            <h3 className="font-sans text-[10px] font-bold tracking-wider text-[#7a4d38] uppercase mb-2">Internal notes</h3>
            <textarea
              value={notesValue}
              ...
              rows={4}
              className="font-sans text-[13px] text-[#2a0f05] w-full rounded-xl px-4 py-3 outline-none resize-y"
            ...
            {notesDirty && (
              <div className="mt-2 flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={notesSaving}
                  className="font-sans text-[12px] text-[#7a4d38] hover:text-[#2a0f05]"
                >
                  Cancel
                </button>
              </div>
          ...
          {t.statusEvents.length > 0 && (
            <section>
              <h3 className="font-sans text-[10px] font-bold tracking-wider text-[#7a4d38] uppercase mb-2">Status history</h3>
              <ul className="rounded-xl overflow-hidden" style={{ border: "1px solid #f0e8dc" }}>
                {t.statusEvents.map((evt) => (
                  <li
                    key={evt.id}
                    className="flex items-center justify-between px-4 py-2 border-b border-[#f0e8dc] last:border-b-0 font-sans text-[12px]"
                  ...
                  >
                    ...
                    <span className="text-[#7a4d38]">{formatDate(evt.createdAt)}</span>
                  </li>
                ))}
              </ul>
            </section>
