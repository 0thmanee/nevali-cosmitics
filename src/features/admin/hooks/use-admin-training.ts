...
export const adminTrainingProgramsQueryKey = ["admin", "training", "programs"] as const;
...
export function useAdminTrainingPrograms(statusFilter?: "DRAFT" | "PUBLISHED") {
  return useQuery({
    ...
    queryFn: () => listTrainingProgramsForAdmin(statusFilter ? { status: statusFilter } : {}),
...
export function useCreateTrainingProgram() {
  ...
  return useMutation({
    mutationFn: (data: CreateTrainingProgramInput) => createTrainingProgram(data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: adminTrainingProgramsQueryKey }),
  });
}
...
export function useUpdateTrainingProgram(programId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: UpdateTrainingProgramInput) => updateTrainingProgram(programId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: adminTrainingProgramsQueryKey });
      queryClient.invalidateQueries({ queryKey: adminTrainingProgramQueryKey(programId) });
    },
  });
}
...
export function useCreateTrainingModule() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: adminTrainingProgramsQueryKey });
      queryClient.invalidateQueries({ queryKey: adminTrainingProgramQueryKey(variables.programId) });
    },
  });
}
...
export function useUpdateTrainingModule(programId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ moduleId, data }: { moduleId: string; data: Parameters<typeof updateTrainingModuleAction>[1] }) =>
...
export function useDeleteTrainingModule(programId: string) {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: adminTrainingProgramQueryKey(programId) }),
  });
}
...
export function useAddTrainingProgramMedia(programId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: CreateTrainingProgramMediaInput) => addTrainingProgramMedia(data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: adminTrainingProgramQueryKey(programId) }),
  });
}
...
export function useAssignProgramToOrganization(programId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: CreateEnrollmentInput) => assignProgramToOrganization(input),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: adminTrainingProgramQueryKey(programId) }),
  });
}
