...
export function adminProductsFilterQueryKey(status: "PENDING" | "APPROVED" | "REJECTED" | "ALL", organizationId?: string | null) {
  return [...adminProductsQueryKey, status === "ALL" ? "all" : status, organizationId ?? "all"] as const;
}

export function useAdminProducts(status: "PENDING" | "APPROVED" | "REJECTED" | "ALL" = "PENDING", organizationId?: string | null) {
  return useQuery({
    queryKey: adminProductsFilterQueryKey(status, organizationId),
    queryFn: () =>
      listProductsForAdmin(status === "ALL" ? { organizationId: organizationId ?? undefined } : { status, organizationId: organizationId ?? undefined }),
...
export function useAdminProductCounts(organizationId?: string | null) {
  return useQuery({
    queryKey: [...adminProductsQueryKey, "counts", organizationId ?? "all"] as const,
