...
import { adminProductsQueryKey, adminProductQueryKey } from "./use-admin-products";
...
export function adminCertificationsFilterKey(filters: {
  ...
  return [...adminCertificationsQueryKey, filters.organizationId ?? "all", filters.status ?? "all", filters.productId ?? "all"] as const;
}

export function useAdminCertifications(filters: {
  organizationId?: string | null;
  status?: "PENDING" | "APPROVED" | "REJECTED";
  productId?: string | null;
} = {}) {
  return useQuery({
    ...
    queryFn: () => listCertificationsForAdmin({ ...filters, organizationId: filters.organizationId ?? undefined }),
...
export function useAdminCertificationCounts(organizationId?: string | null) {
  return useQuery({
    queryKey: [...adminCertificationsQueryKey, "counts", organizationId ?? "all"] as const,
...
export function useSetCertificationStatus() {
  ...
  return useMutation({
    mutationFn: (input: SetCertificationStatusInput) => setCertificationStatus(input),
