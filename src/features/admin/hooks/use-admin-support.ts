...
export function adminSupportTicketsFilterKey(status: "OPEN" | "IN_REVIEW" | "RESOLVED" | "ALL", organizationId?: string | null) {
  return [...adminSupportTicketsQueryKey, status === "ALL" ? "all" : status, organizationId ?? "all"] as const;
}

export function useAdminSupportTickets(status?: "OPEN" | "IN_REVIEW" | "RESOLVED", organizationId?: string | null) {
  return useQuery({
    queryKey: adminSupportTicketsFilterKey(status ?? "ALL", organizationId),
    queryFn: () => listSupportTicketsForAdmin(status ? { status, organizationId: organizationId ?? undefined } : { organizationId: organizationId ?? undefined }),
...
export function useUpdateSupportTicketStatus() {
  ...
  return useMutation({
    mutationFn: (input: UpdateSupportTicketStatusInput) => updateSupportTicketStatus(input),
...
export function useAdminSupportTicketCounts(organizationId?: string | null) {
  return useQuery({
    queryKey: [...adminSupportTicketsQueryKey, "counts", organizationId ?? "all"] as const,
...
export function useUpdateSupportTicketAdminNotes() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: UpdateSupportTicketAdminNotesInput) => updateSupportTicketAdminNotes(input),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: adminSupportTicketsQueryKey });
      queryClient.invalidateQueries({
        queryKey: adminSupportTicketDetailQueryKey(variables.ticketId),
