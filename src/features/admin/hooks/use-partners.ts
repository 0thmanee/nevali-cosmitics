...
export function adminPartnersPaginatedQueryKey(page: number, pageSize: number, organizationId?: string | null) {
  return [...adminPartnersQueryKey, "paginated", page, pageSize, organizationId ?? "all"] as const;
}
...
export function usePartners(organizationId?: string | null) {
  return useQuery({
    ...
    queryFn: () => listPartners({ organizationId: organizationId ?? undefined }),
  });
}

export function usePartnersPaginated(page: number, pageSize: number = 10, organizationId?: string | null) {
  return useQuery({
    queryKey: adminPartnersPaginatedQueryKey(page, pageSize, organizationId),
    queryFn: () => listPartnersPaginated({ page, pageSize, organizationId: organizationId ?? undefined }),
...
export function useUpdatePartner() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ userId, data }: { userId: string; data: UpdatePartnerInput }) =>
