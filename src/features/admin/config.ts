...
export const PAGE_SUBTITLE: Record<string, string> = {
	...
	"/admin/analytics":
		"Catalog checkout: order volumes, status mix, and revenue by brand.",
	"/admin/users": "Activate artisan accounts and manage access.",
	"/admin/products": "Approve or reject artisan product listings.",
	"/admin/orders":
		"Guest checkout orders from the public catalog (buyer contact & lines).",
	"/admin/certifications":
		"Approve or reject certification documents (separate from product approval).",
	"/admin/training":
		"Create and manage training programs; assign programs to artisans.",
...
export function getPageSubtitle(pathname: string): string {
	if (pathname in PAGE_SUBTITLE)
		return PAGE_SUBTITLE[pathname as keyof typeof PAGE_SUBTITLE] ?? "";
	if (pathname.startsWith("/admin/users/"))
		return "View and manage artisan account and profile.";
	if (pathname.startsWith("/admin/products/"))
		return "View product details and linked certifications; approve or reject product and certs separately.";
	if (pathname.startsWith("/admin/training/"))
		return "Edit program, modules, and media; assign to organizations.";
	if (pathname.startsWith("/admin/support/"))
		return "View message, update status, and add internal notes.";
	if (pathname === "/admin/orders") return PAGE_SUBTITLE["/admin/orders"] ?? "";
	return (
		PAGE_SUBTITLE["/admin"] ??
		"Artisan & product management, certification, analytics, training."
	);
}
