...
import { producerProductsQueryKey, producerProductQueryKey } from "./use-products";

export const producerCertificationsQueryKey = ["artisan", "certifications"] as const;
...
export function useAddCertification() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: producerCertificationsQueryKey });
      if (variables.productId) {
        queryClient.invalidateQueries({ queryKey: producerProductQueryKey(variables.productId) });
...
export function useRemoveCertification() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (certificationId: string) => removeCertification(certificationId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: producerCertificationsQueryKey });
      queryClient.invalidateQueries({ queryKey: producerProductsQueryKey });
    },
