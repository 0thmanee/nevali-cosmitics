...
export const producerTrainingProgramsQueryKey = ["artisan", "training", "programs"] as const;
...
export function useEnrollInProgram() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: producerTrainingProgramsQueryKey });
    },
  });
}
...
export function useUpdateEnrollmentProgress() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: UpdateEnrollmentProgressInput) => updateMyEnrollmentProgress(input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: producerTrainingProgramsQueryKey });
