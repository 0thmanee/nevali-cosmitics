...
import type { CreateProducerArticleInput, UpdateProducerArticleInput } from "~/app/api/articles/schemas/article.schema";
...
export function useUpdateArticle() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: producerArticlesQueryKey });
      queryClient.invalidateQueries({ queryKey: producerArticleQueryKey(variables.id) });
    },
  });
}
