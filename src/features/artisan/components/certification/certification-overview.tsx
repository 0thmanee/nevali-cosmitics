...
function StatusChip({ label, count, color, dot }: { label: string; count: number; color: string; dot: string }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="font-sans text-[12px] text-[#727272]">{label}</span>
...
function ProgressBar({ approved, total, color }: { approved: number; total: number; color: string }) {
  ...
  return (
    <div className="w-full h-1.5 rounded-full bg-[#d8d0c4] overflow-hidden">
      <div
...
function CertTypeRow({
  ...
  return (
    <div className="flex flex-col gap-3 p-4 rounded-sm" style={{ border: "1px solid #d8d0c4", background: "#ffffff" }}>
      ...
      {total === 0 ? (
        <p className="font-sans text-[12px] text-[#727272] italic">{emptyHint}</p>
      ) : (
        <>
          ...
          <div className="flex items-center gap-5 flex-wrap">
            ...
            <StatusChip label="Pending"  count={pending}  color="#727272" dot="#727272" />
...
function StatNumber({ value, label, sub }: { value: number; label: string; sub?: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      <span className="font-serif font-bold text-[28px] text-[#000000] leading-none">{value}</span>
      <span className="font-sans text-[12px] font-semibold text-[#000000]">{label}</span>
      {sub && <span className="font-sans text-[11px] text-[#727272]">{sub}</span>}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {([
          { value: certifiedCount, label: "Certified products", icon: <Award      size={18} strokeWidth={1.5} />, bg: "rgba(0,0,0,0.08)",   color: "#000000" },
          ...
          <div key={label} className="rounded-sm px-4 py-3.5 flex items-center gap-3" style={{ background: "white", border: "1px solid #d8d0c4" }}>
            ...
            <div>
              ...
              <p className="font-sans text-[11px] text-[#727272] mt-0.5">{label}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-sm px-4 py-4 flex flex-col gap-3" style={{ background: "white", border: "1px solid #d8d0c4" }}>
            <div className="flex items-center gap-2 pb-3 border-b" style={{ borderColor: "#d8d0c4" }}>
              ...
              <span className="font-sans font-semibold text-[13px] text-[#000000]">Certified products</span>
            </div>
            <div className="flex items-end gap-2">
              <span className="font-serif font-bold text-[32px] leading-none text-[#000000]">{certifiedCount}</span>
              <span className="font-sans text-[12px] text-[#727272] mb-1">product{certifiedCount !== 1 ? "s" : ""} approved</span>
            </div>
            <p className="font-sans text-[11px] text-[#727272] leading-relaxed">
          ...
          <div className="rounded-sm px-4 py-4 flex flex-col gap-2" style={{ background: "rgba(201,145,61,0.06)", border: "1px solid rgba(201,145,61,0.20)" }}>
            <div className="flex items-center gap-2">
              <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
                ...
                <path d="M7 4v3.5l2 1.5" stroke="#727272" strokeWidth="1.2" strokeLinecap="round" />
              </svg>
              <span className="font-sans text-[11px] font-bold text-[#727272] uppercase tracking-wide">Renewal reminder</span>
            </div>
            <p className="font-sans text-[12px] text-[#727272]/80 leading-relaxed">
