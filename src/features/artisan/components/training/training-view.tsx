  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <p className="font-sans text-sm text-[#7a4d38]">Loading programs…</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-1">
        {TABS.map((tab) => (
          <button
            ...
            style={activeTab === tab ? { background: "#1a0500", color: "white" } : { background: "white", color: "#7a4d38", border: "1px solid #f0e8dc" }}
          >
            {tab}
            <span
              className="ml-1.5 text-[10px] font-bold rounded-full px-1.5 py-0.5"
              style={activeTab === tab ? { background: "rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.8)" } : { background: "rgba(26,5,0,0.08)", color: "#7a4d38" }}
            >
              {counts[tab]}
            </span>
          </button>
      ...
      <div className="flex flex-col gap-3">
        {filtered.length === 0 ? (
          <div className="rounded-xl px-5 py-8 text-center" style={{ background: "white", border: "1px solid #f0e8dc" }}>
          ...
          filtered.map((program) => {
            ...
            const color = LEVEL_COLORS[program.level] ?? "#C8963C";
            ...
            return (
              <div key={program.id} className="rounded-xl overflow-hidden transition-all" style={{ background: "white", border: "1px solid #f0e8dc" }}>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <h3 className="font-serif font-bold text-[15px] text-[#2a0f05] leading-tight">{program.name}</h3>
                        ...
                        <p className="font-sans text-[12px] text-[#7a4d38]">{program.provider} · {program.category}{program.durationLabel ? ` · ${program.durationLabel}` : ""}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {status === "IN_PROGRESS" && (
                          <Link href={`/artisan/training/${program.id}`} className="font-sans text-[12px] font-semibold rounded-xl px-4 py-2 transition-colors inline-block" style={{ background: "#1a0500", color: "white" }}>Continue →</Link>
                        )}
                        {status === "AVAILABLE" && (
                          <button
                            type="button"
                            ...
                            style={{ background: "#FAF5EE", color: "#7b2d1e", border: "1px solid #f0e8dc" }}
                          >
                            {enrollMutation.isPending ? "Starting…" : "Start"}
                          </button>
                        )}
                        {status === "COMPLETED" && (
                          <span className="font-sans text-[12px] font-semibold rounded-xl px-4 py-2 inline-block" style={{ background: "rgba(200,150,60,0.1)", color: "#C8963C", border: "1px solid rgba(200,150,60,0.2)" }}>Completed</span>
                        )}
                        <button type="button" onClick={() => setExpanded(isExpanded ? null : program.id)} className="w-8 h-8 rounded-xl flex items-center justify-center transition-colors" style={{ background: "#F5F0E8", border: "1px solid #f0e8dc" }}>
                          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ transform: isExpanded ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}>
                            <path d="M2 4l4 4 4-4" stroke="#7a4d38" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                          </svg>
                        </button>
                      </div>
                    </div>
                    {status === "IN_PROGRESS" && (
                      <div className="mt-3">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="font-sans text-[11px] text-[#7a4d38]">{modulesCompleted} of {totalModules} modules completed</span>
                          <span className="font-sans text-[11px] font-semibold" style={{ color }}>{progress}%</span>
                        </div>
                        <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "#FAF5EE" }}>
                        ...
                        <p className="font-sans text-[11px] text-[#7a4d38] mt-1.5">
                          Up next: <Link href={`/artisan/training/${program.id}`} className="font-semibold text-[#2a0f05] hover:underline">Open program</Link>
                        </p>
                      </div>
                    )}
                    {status === "COMPLETED" && (
                      <div className="mt-2 flex items-center gap-1.5">
                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                          <circle cx="6" cy="6" r="5" stroke="#C8963C" strokeWidth="1.2" />
                          <path d="M3.5 6l1.8 1.8 3.2-3.6" stroke="#C8963C" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        <span className="font-sans text-[11px] font-semibold text-[#C8963C]">Completed · All {totalModules} modules</span>
                      </div>
                    )}
                  </div>
                </div>
                {isExpanded && (
                  <div className="px-5 pb-5 border-t" style={{ borderColor: "#f0e8dc" }}>
                    <div className="pt-4 flex flex-col gap-3">
                      {program.description && <p className="font-sans text-[13px] text-[#7a4d38] leading-relaxed">{program.description}</p>}
                      <div className="flex flex-wrap gap-2">
                        {[
                          ...
                          <div key={d.label} className="rounded-xl px-3 py-2" style={{ background: "#F5F0E8", border: "1px solid #f0e8dc" }}>
                            <p className="font-sans text-[9px] font-bold tracking-wider text-[#7a4d38] uppercase">{d.label}</p>
                            <p className="font-sans text-[12px] font-semibold text-[#2a0f05] mt-0.5">{d.value}</p>
                          </div>
                        ))}
                      </div>
                      {status === "AVAILABLE" && (
