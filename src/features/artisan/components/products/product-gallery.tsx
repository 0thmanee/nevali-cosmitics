  ...
  if (images.length === 0) {
    return (
      ...
      >
        ...
        <p className="font-sans text-sm text-[#7a4d38]">No images yet</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-sm" style={cardStyle}>
      <div className="px-6 py-4 border-b border-[#f0e8dc]">
        <h2 className="font-serif font-bold text-[15px] text-[#2a0f05]">Gallery</h2>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {images.map((img, i) => (
              <button
                ...
                className="w-auto h-48 rounded-lg overflow-hidden shrink-0 transition-opacity focus:outline-none focus:ring-2 focus:ring-[#7b2d1e]/30"
                style={{
                  border: selectedIndex === i ? "2px solid #7b2d1e" : "1px solid #f0e8dc",
