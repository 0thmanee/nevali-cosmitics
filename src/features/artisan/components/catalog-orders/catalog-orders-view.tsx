    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-hidden rounded-sm border border-cream-dark bg-white">
        <div className="border-b border-cream-dark px-5 py-4">
          <h2 className="font-serif text-[15px] font-bold text-text-dark">{t("catalogOrdersView.ordersSectionTitle")}</h2>
          <p className="mt-0.5 font-sans text-[11px] text-stone-500">{t("catalogOrdersView.ordersSectionSubtitle")}</p>
        </div>

        {ordersLoading ? (
          <div className="flex items-center justify-center px-5 py-12">
            <p className="font-sans text-sm text-stone-500">{t("catalogOrdersView.ordersLoading")}</p>
          </div>
        ) : ordersError ? (
          <div className="px-5 py-6">
            <p className="font-sans text-sm text-red-600">
              {ordersErr instanceof Error ? ordersErr.message : t("catalogOrdersView.ordersFailedLoad")}
            </p>
          </div>
        ) : shopOrders.length === 0 ? (
          <div className="px-5 py-14 text-center">
            <p className="font-sans text-sm text-stone-500">{t("catalogOrdersView.ordersEmpty")}</p>
          </div>
        ) : (
              ...
              <li key={order.id}>
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 border-t border-cream-dark/60 pt-2">
                    ...
                    <span className="font-sans text-[11px] font-semibold text-forest-mid">{t("catalogOrdersView.rowCta")} →</span>
                  </div>
                </Link>
              </li>
            ))}
