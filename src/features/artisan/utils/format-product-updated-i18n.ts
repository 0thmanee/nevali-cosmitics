...
export function formatProductUpdatedRelative(
	date: Date,
	t: Translator,
): string {
	...
	if (days < 7)
		return interpolate(t("artisanProductsTable.updatedDaysAgo"), {
			count: days,
		});
	if (days < 14) return t("artisanProductsTable.updatedOneWeek");
	return interpolate(t("artisanProductsTable.updatedWeeksAgo"), {
		count: Math.floor(days / 7),
	});
}

export function artisanProductStatusLabel(
	status: string,
	t: Translator,
): string {
