...
type CartContextValue = {
  ...
  setQuantity: (productId: string, productVariantId: string, quantity: number) => void;
  ...
  return raw
    ...
    .map((x) => {
      ...
      const minOrderQuantity =
        typeof x.minOrderQuantity === "number" && Number.isFinite(x.minOrderQuantity)
          ? x.minOrderQuantity
          : 1;
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const setQuantity = useCallback((productId: string, productVariantId: string, quantity: number) => {
  ...
  const removeLine = useCallback((productId: string, productVariantId: string) => {
    const key = cartLineKey({ productId, productVariantId });
  ...
  const value = useMemo(
    ...
    [lines, ready, addLine, setQuantity, removeLine, clearCart, totalQuantity, subtotalMad],
  );
