# Origine Maroc ↔ nevaliCosmetics parity matrix

This document maps **Origine Maroc–style marketplace behavior** to **nevaliCosmetics** implementation status. Use it to track intentional differences (branding, extra features) versus gaps to close.

**Reference note:** The `origine-maroc/` snapshot is not always present in this repo (and may be excluded from TypeScript builds). When it is missing, this matrix is maintained from **nevaliCosmetics `src/`** plus prior Origine audits. Re-validate against a fresh Origine checkout when needed.
...
| **nevaliCosmetics-only** | Not part of the minimal Origine reference; product choice. |
...
| Origine (typical) | nevaliCosmetics | Status | Notes |
|-------------------|------------|--------|--------|
| `/producer/*` partner app | `/artisan/*` | **Aligned** (renamed) | Legacy `/producer/*` may redirect via `next.config` (see `INTEGRATION_STATUS.md`). |
| Public `/products`, `/products/[id]` | Same | **Aligned** | nevaliCosmetics branding, terracotta theme. |
...
| Capability | nevaliCosmetics location | Status | Notes |
|------------|---------------------|--------|--------|
...
| Variant-aware “hero” image on cards (Origine helper) | `firstImageUrl` = first gallery image by sort | **Partial** | Origine could prefer an image linked to a default/selected variant; nevaliCosmetics uses ordered gallery + mapper. Optional refinement. |

---

## 3. Admin & commerce rules

| Capability | nevaliCosmetics | Status | Notes |
|------------|------------|--------|--------|
...
| **Card payments (Stripe)** | Checkout Session, webhook, `PENDING_PAYMENT` → `CONFIRMED` | **nevaliCosmetics-only** | Origine reference was **record order + method**, not PSP capture. |
...
| Card: Stripe redirect, emails after payment | `stripe-shop-order.ts`, webhook, success sync | **nevaliCosmetics-only** | Stronger than Origine snapshot. |
...
| RFQ/thread email matrix | Broader than minimal Origine | **nevaliCosmetics-only** | Not required for “catalog parity.” |

---

## 6. Domains nevaliCosmetics has beyond minimal Origine
...
**nevaliCosmetics matches Origine on partner catalog depth, admin payment rules, guest checkout, and order notifications.** The main **catalog parity gap** vs the Origine reference is **public full-text search (`q`)** on `/products`. **Stripe** and **RFQ/buyer/training** are **nevaliCosmetics extensions**, not Origine clones.
