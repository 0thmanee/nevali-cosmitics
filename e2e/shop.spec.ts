...
/**
 * Resolve the first product's URL on /products and hard-navigate to it.
 * A hard navigation (vs. a soft client-side click) avoids a transient RSC
 * "network error" the Turbopack dev server can throw under heavy serial load.
 */
async function gotoFirstProduct(page: Page) {
	...
	const href = await page
		...
		.first()
		.getAttribute("href");
	expect(href, "expected at least one product link on /products").toBeTruthy();
