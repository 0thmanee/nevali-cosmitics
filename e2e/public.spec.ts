...
test.describe("SEO surfaces", () => {
	...
	test("product detail has canonical + OG + Product JSON-LD", async ({
		...
		// Resolve the first product URL, then hard-navigate to it so the
		// server-rendered canonical/OG/JSON-LD are guaranteed present (a soft client
		// navigation can read the head before the new page's metadata is applied).
		...
		expect(
			href,
			"expected at least one product link on /products",
		).toBeTruthy();
		...
		await expect
			.poll(async () => {
				...
				return texts.some((t) => /"@type"\s*:\s*"Product"/.test(t));
			})
			.toBe(true);
	});
});
